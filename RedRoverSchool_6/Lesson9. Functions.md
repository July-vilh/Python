...
ОТЛИЧИЕ return от print:

- под return мы как бы сохраняем какое-то действие (например умножение) которое потом мы можем использовать в других частях функции
...
``` 
> RESULT: 10
> тут типо принт, тк это умножение по сути нигде больше не сипользуется (т.е. то-то более простое)
